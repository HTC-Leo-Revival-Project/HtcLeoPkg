  ...
  )
{
  ...
  //
  ConOutPrivate->TextOutQueryData[0].Columns  = 40;
...
/**
  ...
  Because we are running on a calculator, 80x25 is 40x12 here.

  @param TextOutModeMap  Current text out mode map, begin with the mode 40x12
  @param NewlyAddedMap   New text out mode map, begin with the mode 40x12
  ...
  )
{
  ...
  //
  // Because this is a calculator, 80x25 is 40x12
  //
  Index           = 2;
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    //
    // if current mode setting is failed, default 40x12 mode will be set.
    //
    Status = TextOut->SetMode (TextOut, BaseMode);
    ASSERT(!EFI_ERROR(Status));

    Status = PcdSet32S (PcdConOutColumn, 40);
    ASSERT(!EFI_ERROR(Status));
    Status = PcdSet32S (PcdConOutRow, 12);
    ASSERT(!EFI_ERROR(Status));
  }
  ...
  )
{
  ...
  if (CurrentNumOfConsoles == 0) {
    ...
    Private->TextOutQueryData[0].Columns  = 40;
    Private->TextOutQueryData[0].Rows     = 12;
